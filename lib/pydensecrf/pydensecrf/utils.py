...
def unary_from_labels(labels, n_labels, gt_prob, zero_unsure=True):
    ...
    templabels[np.nonzero(templabels==0)]=1
    U[templabels - 1 if zero_unsure else labels, np.arange(U.shape[1])] = p_energy

    # Overwrite 0-labels using uniform probability, i.e. "unsure".
    if zero_unsure:
        U[labels[np.nonzero(labels==0)],np.arange(U.shape[1])[np.nonzero(labels==0)]] = -np.log(1.0 / n_labels)
